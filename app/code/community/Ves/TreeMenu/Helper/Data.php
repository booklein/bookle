...
class Ves_TreeMenu_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    function get($attributes = NULL) {
        ...
        foreach ($arrayParams as $var) {
            ...
            foreach ($tags as $tag) {
                if (Mage::getStoreConfig("ves_treemenu/$tag/$var") != "") {
                    $data[$var] = Mage::getStoreConfig("ves_treemenu/$tag/$var");
                }
            }
    ...
    public function getImageUrl($url = null) {
        return Mage::getSingleton('ves_treemenu/config')->getBaseMediaUrl() . $url;
    }

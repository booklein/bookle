			...
			if( $files ){
				foreach( $files as $file ){
					if($count <= $limit) {
						...
						$tmp['thumbnailURL'] 	= $mediaURL.$this->resizeImage( $folder.DS.$file, $this->getConfig("thumb_width",200), $this->getConfig("thumb_height",200) );
						$tmp['description'] 	= "";
						$tmp['link'] 			= "";
	 		...
	 		for($i=1; $i<=$limit; $i++) {
				...
				if($image_file) {
					...
					if ($image_file && !preg_match("/^http\:\/\/|https\:\/\//", $image_file)) {
			            $thumbnailurl = Mage::helper("ves_base")->resizeImage($image_file, (int)$this->getConfig("thumb_width",200), (int)$this->getConfig("thumb_height",200));
			        } else {

...
class Ves_TabsHome_Helper_Data extends Mage_Core_Helper_Abstract {
  ...
  public function getTabs(){
    ...
    if($this->getConfig('enable_related')){
      $tabs['related'] = Mage::helper('ves_tabshome')->__('Related');
    }
    if($this->getConfig('enable_upsell')){
...
foreach ($arrayParams as $var) {
            $tags = array('ves_tabshome', 'main_slider','catalog_source_settings'); //group config
            foreach ($tags as $tag) {
              if (Mage::getStoreConfig("ves_tabshome/$tag/$var") != "") {
                    $data[$var] = Mage::getStoreConfig("ves_tabshome/$tag/$var"); //item config
                  }
                }

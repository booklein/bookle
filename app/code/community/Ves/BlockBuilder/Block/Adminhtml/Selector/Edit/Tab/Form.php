...
class Ves_BlockBuilder_Block_Adminhtml_Selector_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset->addField("element_attrs", "text", array(
            "label" => Mage::helper("ves_blockbuilder")->__("Css Attributes"),
            "class" => "form-control",
            "name" => "element_attrs",
            'note' => Mage::helper('ves_blockbuilder')->__('Input css attribute which will apply css code. For example: background-image, color, background-color, font-size, color, font, border-color,..<br/><strong>Css Selector { attribute: value; } </strong><br/>For example: <strong>body{ background-color: #FF0000; }</strong>')
        ));
